...
namespace Utils {
	...
	static const char* VulkanShaderStageToString(VkShaderStageFlagBits InStage)
	{
		switch (InStage)
		{
	...
	static const char* VulkanShaderStageCachedVulkanFileExtension(VkShaderStageFlagBits InStage)
	{
		switch (InStage)
		{
...
namespace Silver {
	...
	VulkanShader::VulkanShader(std::string_view InVertex, std::string_view InFragment)
	{
	...
	std::vector<char> VulkanShader::ReadFileAsBinary(std::string_view InFilepath)
	{
		std::ifstream file(InFilepath.data(), std::ios::ate | std::ios::binary);
		...
		AG_CORE_TRACE("Buffer size: {0}", fileSize);
		AG_CORE_TRACE("Successfully read file: {0}", InFilepath);
		return buffer;
	}

	std::string VulkanShader::ReadFileAsString(std::string_view InFilepath)
	{
		std::string result;
	...
	void VulkanShader::CompileOrGetVulkanBinaries(const std::unordered_map<VkShaderStageFlagBits, std::string>& InShaderSources)
	{
		...
		for (auto&& [stage, source] : InShaderSources)
		{
			...
			std::filesystem::path cachedPath = cacheDirectory / (shaderFilePath.stem().string() + Utils::VulkanShaderStageCachedVulkanFileExtension(stage));
	...
	void VulkanShader::CreateShaderModule(VkShaderStageFlagBits InStage)
	{
	...
	void VulkanShader::Reflect(VkShaderStageFlagBits InStage, const std::vector<uint32_t>& InData)
	{
		...
		AG_CORE_TRACE("VulkanShader::Reflect - {0} {1}", Utils::VulkanShaderStageToString(InStage), filename);
